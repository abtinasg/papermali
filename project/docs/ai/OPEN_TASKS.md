...
### Next action — `stage125-research-design-readiness`

Stage125 is a **Research Design & Data Readiness** stage. It performs **no**
modeling. The frozen research contract is in
[`STAGE125_RESEARCH_DESIGN.md`](STAGE125_RESEARCH_DESIGN.md): incremental blocks
M1 Financial → M2 Market → M3 Parsimonious Macro → M4 Structured Audit/Governance;
M5 Persian-text modeling removed; accessibility < 3 data removed; accessibility = 3
is a pilot gate only (must also pass provenance, `published_at`/`available_at`,
coverage, and event-count Gates).

**Part 0 (done — this documentation lock):** baseline after PR #20 confirmed;
research contract recorded in human docs; Handoff regenerated by the generator;
validator + tests green; PR opened (not merged). No extraction, no target/sample
change, no modeling.
...
approved. Stage125 Part 1 (Data Dictionary & Provenance Contract) must not start
until Gate 125.0 is approved.
