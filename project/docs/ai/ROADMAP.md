...
15. `stage125-part3c-leakage-safe-dataset-finalization` — Leakage-safe dataset finalization under the locked six-month Jalali lag (four Gate B designs audited; timing-eligible analysis-ready subsets; assumed availability operationalized; no Stage126 / modeling) ✅
...
- `stage125-part3c-leakage-safe-dataset-finalization` — Stage125 Part 3C leakage-safe dataset finalization (operationalize six-month Jalali lag on all four frozen Gate B designs; audited pair + timing-eligible analysis-ready surfaces; column-role map; offline deterministic rebuild); **no feature selection, no model fitting, no Stage126**
...
`stage125-part3c-leakage-safe-dataset-finalization` operationalized the lag
into audited pair datasets and timing-eligible leakage-safe analysis-ready
datasets for all four Gate B designs (authorized timing exceptions remain
visible in audit only). Next: `stage125-part4-statistical-analysis-plan`. See
