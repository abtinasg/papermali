...
21. `stage127-m2-market-data-gate` — **ACTIVE (not yet authorized).** point-in-time accessibility/coverage/join/quality/event-count Gate for the pre-specified market block. **No modeling in the Gate.** A failed Gate is a reportable negative result and closes the M2 modeling path.
