...
TEMPLATE_COLUMNS = [
    ...
    "current_prelisting_proxy", "current_first_eligible_year_proxy",
...
def build_template(cfg) -> dict:
    t0 = time.time()
    out = stage_dir()
    df = load_stage123_frozen()
    src_hash = utils.sha256_file(stage123_frozen_path())

    df["_year"] = pd.to_numeric(df["fiscal_year"], errors="coerce").astype("Int64")
    rows = []
    for tk, g in df.groupby("ticker", sort=True):
        ...
        rec.update({
            ...
            "current_prelisting_proxy": prelist,
    ...
    # guarantee blank manual-review cells (never guessed / never from old flags)
    for c in _MANUAL_BLANK:
        template[c] = ""
    template.to_csv(out / "listing_master_template_stage124.csv", index=False,
                    encoding="utf-8-sig")

    # ---- QC / controls ---------------------------------------------------
    ds_tickers = set(df["ticker"].unique())
    tmpl_tickers = set(template["ticker"])
    incomplete = sorted(template.loc[template["company_name"].str.strip() == "",
                                     "ticker"])
    qc = {
        "stage": "stage124_part1_template",
        "n_unique_tickers": int(template["ticker"].nunique()),
        "n_rows_template": int(len(template)),
        "duplicate_ticker_count": int(template["ticker"].duplicated().sum()),
        "covers_all_stage123_tickers": ds_tickers == tmpl_tickers,
        "tickers_added_vs_stage123": sorted(tmpl_tickers - ds_tickers),
        "tickers_missing_vs_stage123": sorted(ds_tickers - tmpl_tickers),
        ...
        "stage123_input_file": stage123_frozen_path().name,
        "stage123_input_sha256": src_hash,
        "assertions": {
            "exactly_130_unique": int(template["ticker"].nunique()) == 130,
            "zero_duplicate_ticker": int(template["ticker"].duplicated().sum()) == 0,
            "coverage_exact": ds_tickers == tmpl_tickers,
            "no_new_or_removed": (not (tmpl_tickers - ds_tickers))
            and (not (ds_tickers - tmpl_tickers)),
            "manual_cells_blank": bool((template[_MANUAL_BLANK] == "").all().all()),
        },
        "notes": [
            ...
            "current_prelisting_proxy / current_first_eligible_year_proxy are the EXISTING "
            "Stage123 proxy values, shown only for comparison during manual review.",
        ],
    }
    qc["overall_pass"] = all(qc["assertions"].values())
    utils.save_json(qc, out / "stage124_template_report.json")
    _metadata(cfg, out, src_hash, t0)
    print(f"[stage124] template built: {qc['n_unique_tickers']} tickers, "
          f"dup={qc['duplicate_ticker_count']}, incomplete_name={qc['n_tickers_incomplete_name']}, "
          f"overall_pass={qc['overall_pass']}")
    return {"template": template, "qc": qc, "out": out}


def _metadata(cfg, out, src_hash, t0):
    try:
        commit = subprocess.run(["git", "rev-parse", "HEAD"], cwd=str(ROOT),
                                capture_output=True, text=True).stdout.strip() or None
    except Exception:
        commit = None
    src_files = ["src/stage124.py", "run_stage124.py"]
    meta = {
        "stage": "stage124_part1_template",
        "stage123_input_file": stage123_frozen_path().name,
        "stage123_input_sha256": src_hash,
        ...
        "source_file_sha256": {f: (utils.sha256_file(ROOT / f) if (ROOT / f).exists()
                                   else None) for f in src_files},
