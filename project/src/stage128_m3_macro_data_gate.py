...
def verify_protected_immutability(
    ...
    added = sorted(set(_tracked_files_under(root, "HEAD")) - set(expected_paths))
    if added:
        raise M3MacroDataGateError(
...
def build_definition_lock(
    ...
    accessible = evidence["any_authoritative_data_evidence_obtained"]
    ...
    for candidate_id in M3_CANDIDATE_IDS:
        ...
        for field in REQUIRED_LOCK_FIELDS:
            ...
            elif field in _CONTRACT_DETERMINED and not accessible:
        ...
        candidates.append({
            ...
            "ambiguity_classes": list(_AMBIGUITY_CLASSES[candidate_id]),
    ...
    return {
        ...
        "lock_note": (
            ...
            "recorded as null and unresolved because no authoritative CBI "
            "artifact could be retrieved (see the raw evidence manifest). No "
            "series was chosen opportunistically, no alternative series was "
            "tried to improve coverage, and no candidate was dropped so that "
            "a smaller block could pass."
        ),
    }
...
def assess_official_evidence(root: Path) -> dict[str, Any]:
    """Summarize the recorded official-source retrieval attempts.

    Reads the committed raw evidence manifest. A probe that returns a WAF
    rejection, a JavaScript bot challenge or a CAPTCHA is NOT data evidence,
    and a response whose bytes differ between two identical requests is not a
    ...
    data_evidence = [p for p in probes
                     if p.get("usable_as_authoritative_data_evidence")]
    reproducible = [p for p in probes if p.get("byte_identical_on_repeat")]
    return {
        "probe_count": len(probes),
        "official_hosts_only": True,
        "unofficial_or_aggregator_source_used": False,
        "sci_substitution_used": False,
        "probes_returning_waf_rejection": sum(
            1 for p in probes if p.get("is_waf_request_rejected")),
        "probes_returning_bot_challenge": sum(
            1 for p in probes if p.get("is_js_bot_challenge")),
        "probes_returning_captcha": sum(
            1 for p in probes if p.get("is_captcha_challenge")),
        "probes_unreachable": sum(
            1 for p in probes if p.get("transport_error")),
        "probes_containing_macro_data_series": sum(
            1 for p in probes if p.get("contains_macro_data_series")),
        "byte_reproducible_probe_count": len(reproducible),
        "authoritative_data_evidence_count": len(data_evidence),
        "any_authoritative_data_evidence_obtained": bool(data_evidence),
        "captcha_never_solved_or_bypassed": True,
        "evidence_note": (
            "Every official CBI URL that responded returned either a WAF "
            "'Request Rejected' page or a JavaScript/CAPTCHA bot-protection "
            "challenge; the Time Series Database hosts did not respond at "
            "all. No probe returned a macro data series. No two identical "
            "requests returned identical bytes, so no reproducible retrieval "
            "path exists. The CAPTCHA was never solved or bypassed."
        ),
    }
...
def evaluate_gate_rules(
    ...
    Missing evidence is recorded as ``None`` (unresolved), never as ``False``
    and never scored as zero.
    """
    have_data = evidence["any_authoritative_data_evidence_obtained"]
    reproducible = evidence["byte_reproducible_probe_count"] > 0
    results: dict[str, Any] = {
        # accessibility score is unresolved, not zero, when no official
        # artifact could be retrieved at all
        "G01": None if not have_data else None,
        "G02": None if not have_data else None,
        "G03": False if (not reproducible and have_data) else None,
        "G04": None,
        "G05": None,
        "G06": None,
        "G07": None,
    ...
    if not have_data:
        results["G03"] = None
    detail = {
        "G01": ("accessibility score unresolved: no official CBI artifact was "
                "retrievable, so no score may be assigned; a missing score is "
                "never scored as zero"),
        "G02": ("authoritative source unresolved: the required authority "
                f"{REQUIRED_AUTHORITY} was reachable only through bot "
                "protection, so no authoritative artifact was obtained"),
        "G03": ("reproducible retrieval path unresolved: identical repeated "
                "requests returned different bytes (per-session challenge "
                "tokens) and no data artifact was ever returned"),
        "G04": ("published_at / available_at unverified: no release metadata "
                "could be retrieved"),
        "G05": ("extraction, unit, calendar and join error control "
                "unresolved: no values were extracted"),
        "G06": ("missing availability means unavailable: applied, but there "
                "are no observations to which to apply it"),
...
def determine_gate_status(
    ...
    if not evidence["any_authoritative_data_evidence_obtained"]:
        reasons.append(
            "no authoritative CBI data artifact was obtained: "
            f"{evidence['probes_returning_waf_rejection']} WAF rejections, "
            f"{evidence['probes_returning_captcha']} CAPTCHA challenges, "
            f"{evidence['probes_unreachable']} unreachable hosts across "
            f"{evidence['probe_count']} official-host probes")
    if evidence["byte_reproducible_probe_count"] == 0:
        reasons.append(
            "no reproducible retrieval path: identical repeated requests to "
            "the same official URL returned different bytes")
...
def build_human_decision_request(
    ...
    return {
        ...
        "why_unresolved": [
            "The frozen Stage125 contracts register the three M3 candidate "
            "names but do not uniquely determine, for any candidate, the "
            "official series identity, the transformation formula, the "
            "transformation window, the calendar, the release-date field, the "
            "as-of selection rule or the revision/vintage policy.",
            "No authoritative Central Bank of Iran artifact could be "
            "retrieved: every responding official URL returned a WAF "
            "rejection or a bot-protection CAPTCHA, and the Time Series "
            "Database hosts did not respond. The CAPTCHA was not solved or "
            "bypassed.",
            "Retrieval is not byte-reproducible: two identical requests to "
            "the same official URL returned different bytes.",
        ],
        ...
        "decisions_required_from_the_human": [
            {
                ...
                "ambiguity_classes": candidate["ambiguity_classes"],
            } for candidate in lock["candidates"]
        ],
...
def build_package(
    ...
    if phase_b_permitted:  # pragma: no cover - unreachable while unresolved
        raise M3MacroDataGateError(
            "Phase B execution path is not implemented in this action: the "
            "definition lock resolved unexpectedly and value-level execution "
            "requires a fresh, human-reviewed authorization.")
...
All {evidence["probe_count"]} probes targeted official `cbi.ir` hosts only. No
