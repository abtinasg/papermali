#!/usr/bin/env python3
"""Runner for Stage126 M1 — Robustness Part 5: persistent-loss robustness target.

Usage:
    python project/run_stage126_m1_robustness_part5_persistent_loss_target.py --build
    python project/run_stage126_m1_robustness_part5_persistent_loss_target.py --check

Explicitly human-authorized. Development folds only. Offline. Deterministic.
Zero network. Only the target changes (to
`FD_target_persistent_loss_robustness_t_plus_1`). No retuning, no
...
Part 6 is not authorized. Stage125 Part 5 is historical and is not a gate.
"""
...
from src import stage126_m1_robustness_part5_persistent_loss_target as p5  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    mode.add_argument("--build", action="store_true",
                      help="offline deterministic build of the Part 5 artifacts")
    mode.add_argument("--check", action="store_true",
                      help="zero-network validation; write nothing; exit 1 on drift")
    args = parser.parse_args(argv)

    print("=" * 70)
    print("Stage126 M1 — Robustness Part 5: Persistent-Loss Robustness Target")
    print("=" * 70)
    ...
    except p5.QCFail as exc:
        print(f"FAIL (fail-closed): {exc}", file=sys.stderr)
        return 1

    qc = result["qc"]
    cmp_ = result["comparison"]
    print(f"Category: {p5.CATEGORY_ID} (changed dimension: {qc['changed_dimension']})")
    print(f"Micro-part: {p5.MICRO_PART_ID}")
    print(f"Sample: {qc['sample']} (unchanged; primary: {qc['primary_sample']})")
    print(f"Primary target: {qc['primary_target']} | Part 5 target: {qc['target']}")
    ...
    t = qc["development_target_transitions"]
    print(
        f"target transitions (primary->persistent): 00={t['primary0_persistent0']} "
        f"01={t['primary0_persistent1']} 10={t['primary1_persistent0']} "
        f"11={t['primary1_persistent1']} | primary_pos={t['primary_positive']} "
        f"persistent_pos={t['persistent_positive']} (net {t['net_positive_delta']:+d})"
    )
    print(
    ...
    print(
        ...
        f"{qc['final_test_aggregate_primary_positive']} pos | persistent-loss "
        f"{qc['final_test_aggregate_persistent_loss_positive']} pos"
    )
    ...
    print("  Part 5 ordering:  " + " > ".join(cmp_["part5_observed_ordering"]))
    print(f"  primary ordering preserved: {cmp_['primary_ordering_preserved']}")
    print("Part 6 authorized: false | final test: locked | "
          "development-only secondary target robustness")
