...
# --------------------------------------------------------------------------- #
# The auto-generated handoff/state files were left byte-for-byte untouched
# --------------------------------------------------------------------------- #
...
def test_handoff_state_and_current_state_are_untouched_by_this_action():
    for rel in ("project/docs/ai/handoff_state.json",
                "project/docs/ai/CURRENT_STATE.md"):
        diff = _git_diff_against_main(rel)
        assert diff == "", (
            f"{rel} must stay byte-for-byte identical to origin/main for "
            f"this design-only contract-lock action; got a non-empty diff:\n"
            f"{diff}")
