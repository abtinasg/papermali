...
def test_expected_active_workstream_advances_to_the_m3_gate():
    ...
    # Stage128 constants stay pinned as history, and so do the Phase 2
    # "…_human_review" and "…_submission_metadata" labels; the LIVE
    # expectation has since advanced to the prepared Zenodo dataset Release
    # Candidate awaiting a human digest review. The submission metadata is
    # still outstanding underneath — the label moved because something new
    # became pending, not because the metadata was supplied.
    assert v.STAGE128_M3_ACTIVE_WORKSTREAM == "stage128_m3_macro_data_gate"
    assert v.expected_active_workstream(_root()) == (
        "stage130_dataset_release_candidate_review")
    assert v.STAGE130_SUBMISSION_METADATA_ACTIVE_WORKSTREAM == (
    ...
    assert v.STAGE128_M3I2_RECOVERY_ACTIVE_WORKSTREAM == (
        "stage128_m3i2_final_official_documentary_recovery")
