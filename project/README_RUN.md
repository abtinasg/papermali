...
| Stage126 | `stage126/` | Human-authorized M1 development-fold tuning: three locked model families (regularized Logistic Regression, Random Forest, XGBoost), selected configurations, development OOF predictions and metrics, final-test lock guard; robustness Part 0 decision lock; robustness Parts 1-4 (target-proximity, listing-Rule-B, expanded Rule A, combined Rule B) development-fold results (sensitivity analysis only); **no full-development refit, no final-test evaluation, Part 5 not authorized** |
